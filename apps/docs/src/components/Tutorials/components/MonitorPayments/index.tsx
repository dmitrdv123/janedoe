  ...
  return (
    <>
      ...
      {(i18n.language.toLocaleLowerCase() === 'en' || SUPPORTED_LANGUAGES.findIndex(lang => lang === i18n.language.toLocaleLowerCase()) === -1) && (
        <>
          <p>
            Let's make the payment. Open the main page <a href={`${config.config?.baseUrlAccount}/app`} target="_blank">JaneDoe</a> and follow the example link provided there
          </p>
          <p>
          ...
          <p>
            Return to the admin UI to the <a href={`${config.config?.baseUrlAccount}/payments`} target="_blank">Payment History</a> page. You will be able to see this payment with amount in cryptocurrency, in fiat currency at the payment time and at the current moment. Please note that you need to check that this payment has received the amount you expect. Because the payment amount is passed in the URL parameters and is not recorded anywhere else.
          </p>
          <p>
      ...
      {i18n.language.toLocaleLowerCase() === 'ru' && (
        <>
          <p>
            Давайте совершим платеж. Откройте главную страницу <a href={`${config.config?.baseUrlAccount}/app`} target="_blank">JaneDoe</a> и перейдите по указанной там ссылке примеру
          </p>
          <p>
          ...
          <p>
            Вернитесь в интерфейс администратора на страницу <a href={`${config.config?.baseUrlAccount}/payments`} target="_blank">История платежей</a>. Вы сможете увидеть этот платеж с суммой в криптовалюте, в фиатной валюте на момент платежа и на текущий момент. Обратите внимание, что вам необходимо проверять, что по данному платежу пришла сумма, которую вы ожидаете. Потому что сумма платежа передается в параметрах URL и нигде больше не фиксируется.
          </p>
          <p>
